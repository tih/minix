			  ...
			  unsigned char *string, unsigned int maxlength)
{
  ...
  if (bytesread < 0)
  {
    fprintf(stderr, "Read from link failed - result = %d\n", bytesread);
    bytesread = 0;
  } else {
    if (count == 0)
...
int		sendiserver(LINK TheLink, unsigned int length, unsigned char *buffer)
{
	...
	do {
	  ...
	  start += written;
	  written = WriteLink(TheLink, &buffer[start],
...
#include "boot.h"
	...
	flag = WriteLink(TheLink, boot,
			 sizeof(boot), TIMEOUT) == sizeof(boot);
    if (flag)
    {
	j = 0;
	...
	if (p->parent != NULL)
	    ...
	    flag = (WriteLink(TheLink, params, 4 * BytesPerWord,
			      TIMEOUT) == (4 * BytesPerWord));
/*	  if (p->tpid == 6) {
		    exit(0);
	}
*/	  for (i = 0; (i < length) && flag; i += count)
	{
		...
		flag = WriteLink(TheLink, &Code[i],
				 (int) count, TIMEOUT) == (int) count;
	}
    }
    return (flag);
